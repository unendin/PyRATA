...
def get_current_pattern_step_offsets(p):
    ...
    if p.lexer.lexpos > len(p.lexer.lexdata):
        ...
        previous_lextoken_end = p.lexer.lexpos - len(
            p.lexer.lexTokenEndDict[p.lexer.lexpos].value)
...
class SyntacticPatternParser(object):
    ...
    def p_single_constraint(self, p):
        ...
        var = {}
        var[indice] = symbols(
            single_constraint_string.replace(' ', '\\ ').replace(':', '\\:'))
        p[0] = var[indice]
    ...
    def __init__(self, **kwargs):
        ...
        self.parser = yacc.yacc(module=self, start='step', errorlog=yacc.NullLogger(),
                                debug=False, **kwargs)
...
if __name__ == '__main__':
    ...
    data = [{'raw': 'The', 'lem': 'the', 'pos': 'DT'},
            ...
            {'raw': 'beautiful', 'lem': 'beautiful', 'pos': 'JJ'}]
    print('Data:', data)
