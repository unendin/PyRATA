    ...
    while cur_pos < len(pattern):
        ...
        if c == '(' and not (in_class_constraint or in_constraint_value):
            ...
            if last_char_was_special or cur_pos == 0:
                ...
                guiguan_nfa_pattern_input[len(guiguan_nfa_pattern_input) - 1] = ''.join(
                    [guiguan_nfa_pattern_input[len(guiguan_nfa_pattern_input) - 1], c])
            last_char_was_special = False
...
def build_matching_result(an_nfa, s, i, j, matcheslist):
    ...
    for l in range(an_nfa.step_counter - 1, -1, -1):  # 3rd argument for the reverse order
        ...
        is_id = an_nfa.step_os_is_leaf[l][last_state_id][0] if \
        an_nfa.step_os_is_leaf[l][last_state_id][0] != -1 else leaf_id
    ...
    for g in (groups_start.keys()):
        current_groups.append(
            [s[groups_start[g]:groups_end[g]], groups_start[g], groups_end[g]])
        # debug group
...
class CompiledPattern(object):
    ...
    # """"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    def compile(self, p, lexicons={}, flags=None, **kwargs):
            ...
            if c == '(':
                ...
                if not last_elem:
                    if cur_pos > start_pos:
                        raise self.InvalidRegexPattern(
                            'qualifier %s cannot be put directly after qualifier %s' % (
                            c, p[cur_pos - 1]))
                    else:
                        raise self.InvalidRegexPattern(
                            'qualifier %s cannot be used to qualify empty entity' % c)
                ...
                last_elem = State.create_char_state(c, symbolic_step_expression,
                                                    single_constraint_tuple_list,
                                                    single_constraint_variable_list,
                                                    list(self.group_pile))
    ...
    # """"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    def search(self, s, mode='greedy', matchMethod=False, fullmatchMethod=False, pos=0,
               endpos=None, **kwargs):
        ...
        while i < endpos:  # data exploration
            ...
            while j < endpos:  # pattern exploration
                ...
                if (
                        self.pattern_must_match_data_start or matchMethod or fullmatchMethod) and i != pos:
                    return None
                ...
                an_nfa.step(c, self.lexicons, flags=self.flags)
                ...
                if not len(an_nfa.cur_states):
                    ...
                    if last_matched_nfa != None:
                        ...
                        #
                        matcheslist = build_matching_result(last_matched_nfa, s,
                                                            last_matched_i,
                                                            last_matched_j, MatchesList())

                        if len(matcheslist) > 0:

                            if (
                                    self.pattern_must_match_data_end or fullmatchMethod) and j + 1 != endpos:
                                return None
                ...
                if an_nfa.contains_matching_state():
                    ...
                    if mode == 'greedy':
                        ...
                        if len(
                                an_nfa.cur_states) >= 2 or an_nfa.have_out_states():  #  FIXME =>2 or =>1
        ...
        if last_matched_nfa != None:
            ...
            #
            matcheslist = build_matching_result(last_matched_nfa, s, last_matched_i,
                                                last_matched_j, MatchesList())
        ...
        while i < endpos:  # i position in data
            ...
            while j < endpos:  # j position in the current explored pattern
                ...
                if an_nfa.contains_matching_state():
                    ...
                    if self.pattern_must_match_data_end:
                        if j + 1 == endpos:
                            matcheslist = build_matching_result(an_nfa, s, i, j,
                                                                matcheslist)
                    else:
                        matcheslist = build_matching_result(an_nfa, s, i, j, matcheslist)
    ...
    def annotate(self, annotation, data, group=[0], action='sub', iob=False, **kwargs):
        ...
        iter = self.finditer(data,
                             **kwargs)  # greedy = True #reversed([finditer(pattern, data)])
        if iter != None:
            size = 0
            for m in iter:
                for g in group:
    ...
    def __repr__(self):
        return ''.join(
            ['<pyrata.compiled_pattern CompiledPattern object; \n\tstarts_wi_data="',
             str(self.pattern_must_match_data_start), '"\n\tends_wi_data="',
             str(self.pattern_must_match_data_end), '"\n\tlexicon="',
             str(self.lexicons.keys()), '"\n\tnfa="\n', str(self.nfa), '\n">'])
